<?php 
	...
	if(isset($_POST['qoute']) && !empty($_POST['qoute'])){
		...
		//THIS IS TO TEST IF THE SENTIMENTAL ANALYSIS IS WORKING AND IT IS (MODIFYING $specificString AND $comment_sentiment == 'neg' OR  'pos' returns everything right)
		/*
		$specificString = "i love it!";
		$sentiment = new \PHPInsight\Sentiment();
		$comment_sentiment = $sentiment->categorise($specificString);
		if ($comment_sentiment == 'pos') { echo 'Positive Sentiment Detected'; } 
		*/


		//sentimental analysis to detect positive comments
		$sentiment = new \PHPInsight\Sentiment();
		$comment_sentiment = $sentiment->categorise($comment); //
	

		//PROCESSLIKE STILL NEEDS FIXING
		/*if ($comment_sentiment == 'pos') {
			$_POST['tweet_id'] = $tweet_id; 
			include('autolike.php');
			if ($user_id && $tweet_id) {
				processLike($user_id, $tweet_id);
			} else {
				// Handle error: missing user_id or tweet_id
				echo "Error: Missing necessary IDs for operation.";
			}
		}
		else{
			//nothing
