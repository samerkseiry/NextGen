...
class Tweet extends User {
    ...
    public static function isRetweet($tweet_id){
        ...
        public static function getTweetLinks($tweet){
            ...
            $tweet = preg_replace("/@([\w]+)/", "<a class='hash-tweet' href='http://localhost/nextgen/$1'>$0</a>", $tweet);
            return $tweet;		
        }
     ...
     public static function countAllTweets() {
        $stmt = self::connect()->prepare("SELECT COUNT(*) as count FROM posts");
        $stmt->execute();
        return $stmt->fetch(PDO::FETCH_OBJ)->count;
    }
    public static function countAllLikes() {
        $stmt = self::connect()->prepare("SELECT COUNT(*) as count FROM likes");
        $stmt->execute();
        return $stmt->fetch(PDO::FETCH_OBJ)->count;
    }
    
    public static function countAllComments() {
        $stmt = self::connect()->prepare("SELECT COUNT(*) as count FROM comments");
        $stmt->execute();
        return $stmt->fetch(PDO::FETCH_OBJ)->count;
    }

    public static function countAllReposts() {
        $stmt = self::connect()->prepare("SELECT COUNT(*) as count FROM retweets");
        $stmt->execute();
        return $stmt->fetch(PDO::FETCH_OBJ)->count;
    }
    ...
    public static function getAllPosts() {
        $stmt = self::connect()->prepare(
            "SELECT p.id, p.user_id, p.post_on, t.status, t.img, u.username 
             FROM posts p 
    ...
    public static function getTweetsByCommunityId($communityId) {
        global $pdo; 
            $stmt = $pdo->prepare("SELECT * FROM tweets WHERE community_id = :community_id ORDER BY timestamp DESC");
        $stmt->execute(['community_id' => $communityId]);
        return $stmt->fetchAll(PDO::FETCH_OBJ); 
    }
    
    
    public static function communityTweets($user_id, $community_id) {
        $stmt = self::connect()->prepare("
            ...
            AND (posts.user_id = :user_id OR posts.user_id IN (
